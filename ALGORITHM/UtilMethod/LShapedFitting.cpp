...
cv::RotatedRect LShapedFIT::FitBox(std::vector<cv::Point2f> *pointcloud_ptr) {
    ...
    if (minimal_cost > (-1.0) * std::numeric_limits<double>::max() &&
        best_theta < std::numeric_limits<double>::max()) { ;  // Do Nothing, Continue Run As Follows.
    } else {
...
double LShapedFIT::calc_nearest_criterion(const cv::Mat &c1, const cv::Mat &c2) {
    std::vector<double> c1_deep;
    std::vector<double> c2_deep;
    std::vector<double> c1_deep_sort;
    std::vector<double> c2_deep_sort;

    for (int i = 0; i < c1.rows; i++) {
        for (int j = 0; j < c1.cols; j++) {
            c1_deep.push_back(c1.at<double>(i, j));
            c2_deep.push_back(c2.at<double>(i, j));
            c1_deep_sort.push_back(c1.at<double>(i, j));
            c2_deep_sort.push_back(c2.at<double>(i, j));
        }
    }

    // sort vector from min to max.
    sort(c1_deep_sort.begin(), c1_deep_sort.end());
    sort(c2_deep_sort.begin(), c2_deep_sort.end());

    int n_c1 = c1_deep.size();
    int n_c2 = c2_deep.size();

    double c1_min = c1_deep_sort[0];
    double c2_min = c2_deep_sort[0];

    double c1_max = c1_deep_sort[n_c1 - 1];
    double c2_max = c2_deep_sort[n_c2 - 1];
...
double LShapedFIT::calc_var(const std::vector<double> &v) {
    ...
    double var = sqrt(acc_var_num / ((v.size() - 1) > 0 ? (v.size() - 1) : 1));
...
cv::RotatedRect LShapedFIT::calc_rect_contour() {
    ...
    vertex_pts_.clear();
    std::vector<cv::Point2f> vertex_pts_2d;

    double top_left_x = 0.0, top_left_y = 0.0;
    calc_cross_point(a_[0], a_[1], b_[0], b_[1], c_[0], c_[1], top_left_x, top_left_y);
    vertex_pts_.emplace_back(top_left_x, top_left_y, minP.z);
    vertex_pts_.emplace_back(top_left_x, top_left_y, maxP.z);
    vertex_pts_2d.emplace_back(top_left_x, top_left_y);

    double top_right_x = 0.0, top_right_y = 0.0;
    calc_cross_point(a_[1], a_[2], b_[1], b_[2], c_[1], c_[2], top_right_x, top_right_y);
    vertex_pts_.emplace_back(top_right_x, top_right_y, minP.z);
    vertex_pts_.emplace_back(top_right_x, top_right_y, maxP.z);
    vertex_pts_2d.emplace_back(top_right_x, top_right_y);

    double bottom_left_x = 0.0, bottom_left_y = 0.0;
    calc_cross_point(a_[2], a_[3], b_[2], b_[3], c_[2], c_[3], bottom_left_x, bottom_left_y);
    vertex_pts_.emplace_back(bottom_left_x, bottom_left_y, minP.z);
    vertex_pts_.emplace_back(bottom_left_x, bottom_left_y, maxP.z);
    vertex_pts_2d.emplace_back(bottom_left_x, bottom_left_y);

    double bottom_right_x = 0.0, bottom_right_y = 0.0;
    calc_cross_point(a_[3], a_[0], b_[3], b_[0], c_[3], c_[0], bottom_right_x, bottom_right_y);
    vertex_pts_.emplace_back(bottom_right_x, bottom_right_y, minP.z);
    vertex_pts_.emplace_back(bottom_right_x, bottom_right_y, maxP.z);
    vertex_pts_2d.emplace_back(bottom_right_x, bottom_right_y);

    return cv::minAreaRect(vertex_pts_2d);
}

std::vector<cv::Point3f> LShapedFIT::getRectVertex() {
    return vertex_pts_;
}

void
LShapedFIT::fitBox(PointCloudTPtr _cloud, ObjectInfo &_cluster) {
    if (_cloud->size() < 3) {
        return;
    }
    _cluster = {};
    std::vector<cv::Point2f> pointCloud2D;
    size_t cloudSize = _cloud->size();
    minP = cv::Point3f(MAX_FLOAT, MAX_FLOAT, MAX_FLOAT);
    maxP = cv::Point3f(-MAX_FLOAT, -MAX_FLOAT, -MAX_FLOAT);
    for (size_t i = 0; i < cloudSize; ++i) {
        pointCloud2D.emplace_back(_cloud->at(i).x, _cloud->at(i).y);
        if (minP.z > _cloud->at(i).z) {
            minP.z = _cloud->at(i).z;
        }
        if (maxP.z < _cloud->at(i).z) {
            maxP.z = _cloud->at(i).z;
        }
    }
    cv::RotatedRect _rotatedRect = this->FitBox(&pointCloud2D);
    std::vector<cv::Point3f> _cornerPoint = this->getRectVertex();
    double d1 = this->dist2Points(_cornerPoint.at(0), _cornerPoint.at(2));
    double d2 = this->dist2Points(_cornerPoint.at(2), _cornerPoint.at(4));
    objectInfo = {};
    _rotatedRect.angle = atan2(_cornerPoint.at(2).y - _cornerPoint.at(0).y, _cornerPoint.at(2).x - _cornerPoint.at(0).x);
    if (d1 > 0 && d2 / d1 > 1.0) {
        objectInfo.heading = _rotatedRect.angle + M_PI_2;
        objectInfo.l = d2;
        objectInfo.w = d1;
        objectInfo.minP.x = (_cornerPoint.at(0).x + _cornerPoint.at(3).x) * 0.5f;
        objectInfo.minP.y = (_cornerPoint.at(0).y + _cornerPoint.at(3).y) * 0.5f;
        objectInfo.minP.z = (_cornerPoint.at(0).z + _cornerPoint.at(3).z) * 0.5f;
        objectInfo.maxP.x = (_cornerPoint.at(4).x + _cornerPoint.at(7).x) * 0.5f;
        objectInfo.maxP.y = (_cornerPoint.at(4).y + _cornerPoint.at(7).y) * 0.5f;
        objectInfo.maxP.z = (_cornerPoint.at(4).z + _cornerPoint.at(7).z) * 0.5f;

        objectInfo.directW.x = (_cornerPoint.at(2).x - _cornerPoint.at(0).x) / objectInfo.w;
        objectInfo.directW.y = (_cornerPoint.at(2).y - _cornerPoint.at(0).y) / objectInfo.w;
        objectInfo.directW.z = (_cornerPoint.at(2).z - _cornerPoint.at(0).z) / objectInfo.w;
    } else {
        objectInfo.heading = _rotatedRect.angle;
        objectInfo.l = d1;
        objectInfo.w = d2;
        objectInfo.minP.x = (_cornerPoint.at(0).x + _cornerPoint.at(7).x) * 0.5f;
        objectInfo.minP.y = (_cornerPoint.at(0).y + _cornerPoint.at(7).y) * 0.5f;
        objectInfo.minP.z = (_cornerPoint.at(0).z + _cornerPoint.at(7).z) * 0.5f;
        objectInfo.maxP.x = (_cornerPoint.at(2).x + _cornerPoint.at(5).x) * 0.5f;
        objectInfo.maxP.y = (_cornerPoint.at(2).y + _cornerPoint.at(5).y) * 0.5f;
        objectInfo.maxP.z = (_cornerPoint.at(2).z + _cornerPoint.at(5).z) * 0.5f;

        objectInfo.directW.x = (_cornerPoint.at(6).x - _cornerPoint.at(0).x) / objectInfo.w;
        objectInfo.directW.y = (_cornerPoint.at(6).y - _cornerPoint.at(0).y) / objectInfo.w;
        objectInfo.directW.z = (_cornerPoint.at(6).z - _cornerPoint.at(0).z) / objectInfo.w;
    }
    objectInfo.h = maxP.z - minP.z;
    objectInfo.x = (_cornerPoint.at(0).x + _cornerPoint.at(5).x) * 0.5;
    objectInfo.y = (_cornerPoint.at(0).y + _cornerPoint.at(5).y) * 0.5;
    objectInfo.z = (_cornerPoint.at(0).z + _cornerPoint.at(5).z) * 0.5;
    objectInfo.cornerPoint = _cornerPoint;
    objectInfo.rotatedRect = _rotatedRect;
    objectInfo.direct.x = (objectInfo.maxP.x - objectInfo.minP.x) / objectInfo.l;
    objectInfo.direct.y = (objectInfo.maxP.y - objectInfo.minP.y) / objectInfo.l;
    objectInfo.direct.z = (objectInfo.maxP.z - objectInfo.minP.z) / objectInfo.l;

    _cluster = objectInfo;
}

double LShapedFIT::dist2Points(const cv::Point3f &p1, const cv::Point3f &p2) {
    return sqrt((p1.x - p2.x) * (p1.x - p2.x) + (p1.y - p2.y) * (p1.y - p2.y) + (p1.z - p2.z) * (p1.z - p2.z));
}

const ObjectInfo &LShapedFIT::getCluster() const {
    return objectInfo;
}

void LShapedFIT::setCluster(const ObjectInfo &_cluster) {
    LShapedFIT::objectInfo = _cluster;
}
